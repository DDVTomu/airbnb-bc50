...
export default function Register() {
  ...
  const registerSchema = Yup.object().shape({
    name: Yup.string()
      .min(3, "Tên quá ngắn!")
      .required("Xin hãy nhập tên người dùng"),
    password: Yup.string()
  ...
  const handleSubmit = async (values: any) => {
    const rawResponse = await updateAPI(
  ...
  //     initialValues: {},
  //     onSubmit: (values) => handleFileSubmit(values),
  //     validate: validateImage,
  //   });

  //   const handleFileSubmit = async (values: any) => {
  //     console.log(values.image);
  //     console.log(Object.values(values.image));
  //     const formData = new FormData();
  //     formData.append("formFile", values.image);

  //     console.log(formData);

  //     const rawResponse = await postImageAPI(
  //       "users/upload-avatar",
  //       values.image,
  //       userData.token
  //     );
  ...
  return (
    <main
      ...
      style={{
        backgroundImage:
          'url("/static/media/logo_login.a444f2681cc7b623ead2.jpg")',
